...
def load_structure_only_data(DATA, OUTPUTS, label):
    ...
    for _,row in df.iterrows():
        ...
        if voxel_key not in voxel_feats:
            missing.append(mut)
            continue
    ...
    if missing:
        print(f"Warning: missing 3D embeddings for {len(missing)} mutants")

    X_voxel = np.stack(X_voxel)
    X_voxel = np.squeeze(X_voxel, axis=1)
    
    print(f"Final voxel feature shape: {X_voxel.shape}")
    return X_voxel, np.array(y)
...
def train_rf(X,y):
    ...
    print(
        f"Structure Only - 5-fold RMSE: {np.mean(rmses):.4f} ± {np.std(rmses):.4f}"
    )
...
def main():
    ...
    print("===== Loading Structure-Only Data =====")
    X_voxel, y = load_structure_only_data(DATA, OUTPUTS, args.label)

    print("\n===== Training 3D CNN for Structure Feature Extraction =====")
    cnn_model = train_cnn(X_voxel, y)
    torch.save(cnn_model.state_dict(), MODELS/"cnn3d_structure_only.pt")

    print("\n===== Extracting 3D Structure Features =====")
    X_struct = extract_struct_features(cnn_model, X_voxel)
    print(f"Extracted structure feature shape: {X_struct.shape}")

    print("\n===== Training Random Forest =====")
    rf_model = train_rf(X_struct, y)
    dump(rf_model, MODELS/"rf_structure_only.joblib")

    print("\n===== Model Saving Complete =====")
    print("CNN weights saved: models/cnn3d_structure_only.pt")
    print("RF model saved: models/rf_structure_only.joblib")
